...
import {DEFAULT_API_URL, DEFAULT_NUMBER_OF_POSTS, DEFAULT_UPDATE_INTERVAL} from '../constants/Config';
...
class Settings extends Component {
    ...
    retrieveSavedConfig() {
        ...
        this.retrieveDataFromAsyncStorage(KEY_NUMBER_OF_POSTS_ASYNCSTORAGE, 'numberOfPosts');
    ...
    async retrieveTimeFromAsyncStorage() {
        const retrievedData = await AsyncStorage.getItem(KEY_UPDATE_INTERVAL_ASYNCSTORAGE);

        if (retrievedData) {
            updateInterval = retrievedData

            const readableTime = this.convertTimeToMMSS(Number.valueOf(retrievedData));
            this.setState({
    ...
    checkData() {
        console.log(LOG_TAG, this.checkUrl(), this.checkNumberOfPosts(), this.checkUpdateInterval())
        if (this.checkUrl() && this.checkNumberOfPosts() && this.checkUpdateInterval()) {
            this.saveConfig();
    ...
    checkUrl() {
        const res = this.state.feedUrl.match(/(http(s)?:\/\/.)?(www\.)?[-a-zA-Z0-9@:%._\+~#=]{2,256}\.[a-z]{2,6}\b([-a-zA-Z0-9@:%_\+.~#?&//=]*)/g);
        if (res == null) {
            this.setState({
    ...
    checkUpdateInterval() {
        ...
        const intervalInSeconds = this.convertHHMMSStoTime(this.state.humanReadableUpdateInterval);
    ...
    resetConfig(){
        this.setState({
            feedUrl: DEFAULT_API_URL,
            numberOfPosts: DEFAULT_NUMBER_OF_POSTS.toString(),
            humanReadableUpdateInterval: this.convertTimeToMMSS(DEFAULT_UPDATE_INTERVAL),
    ...
    async saveConfig() {
        try {
            ...
            await AsyncStorage.setItem(KEY_NUMBER_OF_POSTS_ASYNCSTORAGE, JSON.stringify(this.state.numberOfPosts));
            await AsyncStorage.setItem(KEY_UPDATE_INTERVAL_ASYNCSTORAGE, JSON.stringify(updateInterval));
            console.log(LOG_TAG, 'updateInterval', updateInterval);
        } catch (error) {
                ...
                <View style={styles.container}>
                    ...
                    <View style={styles.buttonWrapper}>
                        <TouchableOpacity
                            style={[
                                styles.button,
                                {backgroundColor: this.state.submitButtonColor, marginVertical: 16}]}
                        ...
                        <TouchableOpacity
                            style={[
                                styles.button,
                                {backgroundColor: colors.CETACEAN_BLUE}]}
